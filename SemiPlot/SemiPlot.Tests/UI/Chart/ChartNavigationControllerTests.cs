...
public sealed class ChartNavigationControllerTests
{
	...
	// Half a layer's ceiling sits well inside it, clear of both the 25% zoom quantisation step and the
	// 10% hysteresis band. The Day row uses 1.2x the hour ceiling: past the band, and still inside the
	// 365-day maximum window at 1024 columns, so the answer comes from the ladder rather than from
	// TrendNavigationModel truncating the width.
	[Theory]
	...
	// The acceptance table of docs/plans/20260810-layer-ladder-spacing.md, pinned at its literal widths.
	// Every width here sits clear of the 10% hysteresis band, so the answer does not depend on which layer
	// the ladder was entered from.
	[Theory]
