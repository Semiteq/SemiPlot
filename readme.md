...
клоны удаляются вместе с прогоном. Две половины teardown проверяет прямо: не осталось ли клонов
`semiplot_clone_*` и отвечает ли ещё контейнер по своему id — утечка делает прогон красным. Раньше
каждый прогон оставлял один «висячий» образ примерно на 436 МБ. Скачанные из реестра образы
(`postgres:17-alpine`, `ghcr.io/semiteq/semibase:latest`) остаются — намеренно.
