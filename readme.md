...
- Окно настроек («Правка → Настройки»): язык, тема и подключение к архиву, включая пароль. Окно
  ...
  запуске. Ключ, которого окно не показывает (например, `schema`), остаётся в своём файле и
  переживает любую перезапись. Комментарии в переписанном файле не сохраняются
