...
public sealed class StartupProbeTests
{
	...
	// host reads as a startup timeout instead of an accepted-then-timed-out connection. Pinned against
	// the connection string the settings actually yield, not against a literal.
	[Fact]
	public void DefaultReadBound_StaysAboveTheConnectTimeout()
	{
		var connectionString = new NpgsqlConnectionStringBuilder(ConnectionSettingsFactory.Create().ConnectionString);
