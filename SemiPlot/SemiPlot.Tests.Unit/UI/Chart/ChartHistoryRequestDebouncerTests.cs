...
public sealed class ChartHistoryRequestDebouncerTests
{
	...
	public async Task StaleResponse_IsDroppedWhenANewerWindowSupersedesAnInFlightQuery()
	{
		...
		// Real schedulers are used because the guard spans a real async boundary, and the two gates the test
		// body awaits run continuations asynchronously so completing one cannot re-enter the Rx pipeline inline.
