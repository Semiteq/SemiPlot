...
| --- | --- | --- |
| `[MEAS:dump-20260805]` | `pg_dump` custom-format archive of the customer's SCADA archive database, taken on a working installation. Two variables (`id` 0 and 1), about two hours of data, twelve project start/stop cycles, values written by a project script. Expanded with `pg_restore -f`. An anonymised 140-row slice is committed at `SemiPlot/SemiPlot.Tests.Unit/Fixtures/real-archive-rows.csv`; the README beside it records the extraction, the identifier substitution and the timestamp offset. | Verified DDL of `trends` and `messages`; daily partition naming; the schema holds the day partitions `tp2026m08d04` to `tp2026m08d06` and `mp2026m08d04` to `mp2026m08d06`, the day before, the day of and the day after the rows' day, 2026-08-05 (`docs/plans/completed/20260810-archive-populator.md`); coarse-layer rows are verbatim copies of raw rows (170 of 170 identical in timestamp, value and quality); layers strictly nested; gap markers replicated into every layer and aligned with `messages` project stop/start rows within 30 ms; `v` never null; change-based archiving writes two rows per change at a 100 ms poll. From the committed slice additionally: a repeated extreme value is represented in the coarse layer by the **later** of the two rows; one coarse row is the last raw row of its minute and is neither an extreme nor a marker, which confirms *last of the period* as one of the selected points; the poll tick jitters — 30 of 34 change rows sit exactly 100 ms after their predecessor and 4 sit 104 to 109 ms after it. |
...
| `[DEC:machine-time-zone]` | The SCADA, its PostgreSQL archive and the viewer share one machine, and an archive is never opened on another. `[MEAS:dump-20260805]` shows the SCADA stamps `t` on its own process clock (gaps align with project stop/start rows in `messages` within 30 ms; day partitions exist for the SCADA's yesterday, today and tomorrow), but not which zone that clock is in: every stamp is zone-less and no row can be compared with a known instant. The clock is taken to be in the machine's Windows zone, and the viewer reads `t` in `TimeZoneInfo.Local`. If every reading on an installation is shifted by a whole number of hours, this decision is wrong there. The check: `SELECT max(t) FROM trends WHERE l = 0;` beside the Windows clock while the SCADA runs. |
