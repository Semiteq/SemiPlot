...
// construction, not by agreement. No other test compares the two paths, so re-splitting them leaves
// every other suite green until a seeded archive is followed and the COPY meets a key it already holds.
...
public sealed class SharedLatticeTests
{
	...
	public void AFollowRunResumingFromASeededEdgeRepeatsNoRowAndLeavesNoHole()
	{
		...
		followed.Select(row => row.Id).Distinct().Order().Should().Equal(edges.Keys.Order());
		...
		foreach (var pen in followed.GroupBy(row => row.Id))
		{
			...
			seam.Should().BeGreaterThanOrEqualTo(TimeSpan.FromMilliseconds(1.0)).And.BeLessThanOrEqualTo(_changeInterval);
		}
	}
}
